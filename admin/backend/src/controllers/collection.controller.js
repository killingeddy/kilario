...
const collectionController = {
  ...
  getById: asyncHandler(async (req, res) => {
    ...
    const collection = await collectionService.getById(parseInt(id, 10), includeProducts);
    return responses.success(res, { collection });
  }),

  create: asyncHandler(async (req, res) => {
    const collection = await collectionService.create(req.body, req.admin.id);
    return responses.created(res, { collection }, 'Collection created successfully');
  }),

  update: asyncHandler(async (req, res) => {
    const { id } = req.params;
    const collection = await collectionService.update(parseInt(id, 10), req.body, req.admin.id);
    return responses.success(res, { collection }, 'Collection updated successfully');
  }),

  toggleActive: asyncHandler(async (req, res) => {
    const { id } = req.params;
    const collection = await collectionService.toggleActive(parseInt(id, 10), req.admin.id);
    const message = collection.is_active ? 'Collection activated' : 'Collection deactivated';
